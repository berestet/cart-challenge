...
angular.
	...
	component('login', {
	  ...
	  controller: ['$http', '$routeParams', '$cookies', 'SelectedItems',
	    function LoginController($http, $routeParams, $cookies, SelectedItems) {
		    const serverUrl = 'http://localhost:8080';

		    const loginUrl = '/users/login';
		    const phoneListUrl = '/index.html#!/phones';
                    	...
                    	switch( response.data ) {
                        	...
                        	case AuthenticationReturnCode.SUCCESS:
                            	...
                            	SelectedItems.setCurrentUser(self.user.userId);
                            	  ...
                            	  var storedCurrentUser = $cookies.getObject('currentUser');
                            	  if( (storedCurrentUser != null) && (storedCurrentUser.userId != null) ) {
                            		  if( self.user.userId == storedCurrentUser.userId ) {
                            			  window.alert("we have previously stored session for " + self.user.userId);
                            		  } else {
                            			  window.alert(self.user.userId + " has no previous session");
                            		  }
                            	  }
                            	  // $cookies.put('currentUser', self.user.userId); // $cookies
